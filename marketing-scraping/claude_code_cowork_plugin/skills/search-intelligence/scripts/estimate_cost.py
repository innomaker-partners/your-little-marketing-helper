...
Pre-run cost estimate from EMPIRICAL rates (not the actor page's advertised rate).
    ...
    python3 estimate_cost.py A1 500          # estimate 500 keywords on A1
    python3 estimate_cost.py B1 1000 --rate 0.0005    # override with a test-calibrated rate
...
def main():
    ...
    stage = args[0]
    if len(args) < 2:
        sys.exit("Provide an expected count, e.g. `python3 estimate_cost.py A1 500`")
    count = float(args[1])
    rate_override = None
    if "--rate" in args:
