...
process looked hung on a run that was actually finished, every foreground
...
def actual_charge_usd(run):
    ...
    Some pay-per-event actors switched to the third mode ("high compute costs
    are now billed as Apify platform usage, passed through at cost; per-ad fee
    reduced"). Charging bare usageTotalUsd there UNDER-reports by the whole
    per-ad portion — the budget would think it spent less than it did and could
    overrun the cap.
    ...
    if not prices or not counts:
        # A PAY_PER_EVENT run with an empty ledger would silently return ~$0,
        # blinding the budget gate. Print a loud warning so a human notices; do
        # NOT invent a charge — warn + return the fallback so the uncertainty is visible.
        print(
            f"[apify_client] WARNING: PAY_PER_EVENT run {run.get('id', '?')} — "
            f"could not price event charges "
            f"(prices_populated={bool(prices)}, counts_populated={bool(counts)}). "
            f"Falling back to usageTotalUsd (${platform:.4f}), which is the "
            f"developer-absorbed compute cost — NOT the event charges the user owes. "
            f"The budget figure for this run is UNRELIABLE and likely UNDER-REPORTS "
            f"the true bill. Verify the true cost on the Apify console.",
            file=sys.stderr,
        )
...
def _charge_settled(run):
    ...
    state. Live runs proved this is not instant: crawlerbros/google-keywords-
    suggest-scraper-pro still had an empty
    chargedEventCounts at terminal+2s, so actual_charge_usd fell back to the
    (also-unsettled) usageTotalUsd and under-charged the budget by the whole
    per-event portion ($0.065 recorded vs $0.231 real, ~3.5x low on that stage).

    Requiring prices AND a positive charged sum (not just a non-empty counts
    dict): chargedEventCounts can be non-empty while pricingPerEvent has not yet
    populated, AND it can read {"<event>": 0} for a beat at terminal before the
    ledger fills. Either case makes actual_charge_usd fall back or price $0, so
    require both prices present and a charged sum > 0.
...
def run_actor(actor, actor_input, memory_mb=4096, timeout_s=1800, on_status=None,
    ...
    # budget (see _charge_settled). Poll until the charge is priceable, bounded
    # by settle_max_s so a genuinely zero-event or slow run still returns. This
    # is a deterministic latency-bound wait, so a bound is correct here (unlike
    # non-deterministic steps, which get no timeout).
    ...
    # actual_charge_usd(run), which
    # reads the run's own pricingInfo + chargedEventCounts. The USD `eventUsage`
    # field settles minutes after SUCCEEDED, but chargedEventCounts is the live
    # ledger and is reliable at terminal, so actual_charge_usd does not need to
...
if __name__ == "__main__":
    # -------------------------------------------------------------------------
    # Offline tests for actual_charge_usd.
    # No network calls. All inputs are crafted run dicts.
    # -------------------------------------------------------------------------
    import io
    ...
    def check_approx(label, got, expected, tol=1e-9):
        global PASS, FAIL
        if abs(got - expected) <= tol:
            print(f"  PASS  {label}  (${got:.4f})")
            PASS += 1
        else:
            print(f"  FAIL  {label}")
            print(f"        got:      ${got:.6f}")
    ...
    # Shared PPE pricing block used across several cases.
    _PPE_PRICING = {
        "pricingModel": "PAY_PER_EVENT",
        "isPPEPlatformUsagePaidByUser": False,
        "pricingPerEvent": {
            "actorChargeEvents": {
                "page-scraped": {"eventPriceUsd": 0.010},
            }
        },
    }

    print("\n=== TEST 1: PPE run — empty chargedEventCounts (before fix) ===")
    # Before fix: would silently return $0.003 (platform fallback) with no warning.
    # After fix: returns $0.003 AND prints a loud warning to stderr.
    run_empty = {
        "id": "test-run-empty",
        "usageTotalUsd": 0.003,
        "chargedEventCounts": {},
        "pricingInfo": _PPE_PRICING,
    }
    stderr_buf = io.StringIO()
    _old_stderr = sys.stderr
    sys.stderr = stderr_buf
    result_empty = actual_charge_usd(run_empty)
    sys.stderr = _old_stderr
    warning_text = stderr_buf.getvalue()

    check_approx("PPE empty counts: fallback to usageTotalUsd ($0.003)", result_empty, 0.003)
    check("PPE empty counts: WARNING printed to stderr",
          "WARNING" in warning_text and "UNRELIABLE" in warning_text, True)
    check("PPE empty counts: mentions run id", "test-run-empty" in warning_text, True)

    print("\n=== TEST 2: PPE run — counts populated (correct path) ===")
    # 1000 pages × $0.010 = $10.000. Developer absorbs platform; isPPE...PaidByUser=False.
    run_correct = {
        "id": "test-run-correct",
        "usageTotalUsd": 0.003,          # developer cost — NOT charged to user
        "chargedEventCounts": {"page-scraped": 1000},
        "pricingInfo": _PPE_PRICING,
    }
    result_correct = actual_charge_usd(run_correct)
    check_approx("PPE populated counts: event bill = $10.000", result_correct, 10.000)

    BEFORE_AFTER_GAP = result_correct - result_empty
    print(f"\n  BEFORE/AFTER:")
    print(f"    Empty counts (before fix behaviour): ${result_empty:.4f}")
    print(f"    Populated counts (correct):          ${result_correct:.4f}")
    print(f"    Gap (under-report):                  ${BEFORE_AFTER_GAP:.4f}")

    print("\n=== TEST 3: PPE — isPPEPlatformUsagePaidByUser=True (platform pass-through) ===")
    # B5 google-ads-scraper mode: event charges + platform usage both billed.
    run_passthrough = {
        "id": "test-run-passthrough",
        "usageTotalUsd": 0.50,
        "chargedEventCounts": {"page-scraped": 500},
        "pricingInfo": {
            **_PPE_PRICING,
            "isPPEPlatformUsagePaidByUser": True,
        },
    }
    result_passthrough = actual_charge_usd(run_passthrough)
    # 500 × $0.010 = $5.00 events + $0.50 platform = $5.50
    check_approx("PPE passthrough: events ($5.00) + platform ($0.50) = $5.50",
                 result_passthrough, 5.50)

    print("\n=== TEST 4: pay-per-compute actor — no PPE pricing block ===")
    run_compute = {
        "id": "test-run-compute",
        "usageTotalUsd": 0.25,
        "chargedEventCounts": {},
        "pricingInfo": {"pricingModel": "PAY_PER_COMPUTE"},
    }
    # For non-PPE: no warning, returns platform usage directly.
    stderr_buf2 = io.StringIO()
    sys.stderr = stderr_buf2
    result_compute = actual_charge_usd(run_compute)
    sys.stderr = _old_stderr
    check_approx("Pay-per-compute: returns usageTotalUsd ($0.25)", result_compute, 0.25)
    check("Pay-per-compute: NO warning printed", stderr_buf2.getvalue() == "", True)

    print("\n=== TEST 5: PPE — no prices in pricingPerEvent (malformed actor config) ===")
    run_no_prices = {
        "id": "test-run-no-prices",
        "usageTotalUsd": 0.001,
        "chargedEventCounts": {"page-scraped": 500},
        "pricingInfo": {
            "pricingModel": "PAY_PER_EVENT",
            "isPPEPlatformUsagePaidByUser": False,
            "pricingPerEvent": {"actorChargeEvents": {}},  # empty prices
        },
    }
    stderr_buf3 = io.StringIO()
    sys.stderr = stderr_buf3
    result_no_prices = actual_charge_usd(run_no_prices)
    sys.stderr = _old_stderr
    check_approx("PPE no prices: fallback to usageTotalUsd", result_no_prices, 0.001)
    check("PPE no prices: WARNING printed", "WARNING" in stderr_buf3.getvalue(), True)

    # ==========================================================================
    # TEST 6 — run_actor spend safety (L2 maxTotalChargeUsd + L3 poll-and-abort).
    # Fully offline: module-level _req and time.sleep are faked; no network. The
    # run_actor function is byte-identical in the ad-intelligence and search-
    # intelligence copies, so this exercises the shared logic for all three.
    # ==========================================================================
    print("\n=== TEST 6: run_actor spend safety (L2 + L3) ===")

    _PPE_RESULT = {
        "pricingModel": "PAY_PER_EVENT",
        "isPPEPlatformUsagePaidByUser": False,
        "pricingPerEvent": {"actorChargeEvents": {"result": {"eventPriceUsd": 0.01}}},
    }

    def _mk_run(status, n_results, rid="r1"):
        # charge = n_results * $0.01, priced by actual_charge_usd.
        return {"id": rid, "status": status, "usageTotalUsd": 0.0,
                "chargedEventCounts": {"result": n_results},
                "pricingInfo": _PPE_RESULT, "defaultDatasetId": "ds1"}

    class _FakeApify:
        """Stateful fake for _req: POST start -> states[0]; each GET advances one
        state (clamped at the last); POST .../abort records that we aborted."""
        ...
        def req(self, method, path, body=None, **kw):
            if method == "POST" and "/abort" in path:
                self.aborted = True
                return {"data": self.states[min(self.gi, len(self.states) - 1)]}
            if method == "POST":
                self.start_path = path
                self.gi = 0
                return {"data": self.states[0]}
            self.gi += 1
            return {"data": self.states[min(self.gi, len(self.states) - 1)]}
    ...
    _ACTOR_MIN_CACHE["acme/act"] = 0.0  # no floor -> Apify cap == the forecast ceiling
    try:
        # L2a: cap present, formatted to 4 decimals.
        fake = _FakeApify([_mk_run("SUCCEEDED", 3)])
        globals()["_req"] = fake.req
        run_actor("acme/act", {"x": 1}, max_charge_usd=0.05)
        check("L2: start query carries maxTotalChargeUsd=0.0500",
              "maxTotalChargeUsd=0.0500" in (fake.start_path or ""), True)

        # L2b: $0.01 floor stops a tiny forecast rounding to 0.0000.
        fake = _FakeApify([_mk_run("SUCCEEDED", 1)])
        globals()["_req"] = fake.req
        run_actor("acme/act", {"x": 1}, max_charge_usd=0.001)
        check("L2: tiny forecast floored to maxTotalChargeUsd=0.0100",
              "maxTotalChargeUsd=0.0100" in (fake.start_path or ""), True)

        # L2c: no cap passed -> query omits the param entirely.
        fake = _FakeApify([_mk_run("SUCCEEDED", 1)])
        globals()["_req"] = fake.req
        run_actor("acme/act", {"x": 1})
        check("L2: no cap -> query omits maxTotalChargeUsd",
              "maxTotalChargeUsd" not in (fake.start_path or ""), True)

        # L3a: live charge crosses the ceiling mid-run -> we abort ourselves.
        # RUNNING $0.02 -> RUNNING $0.06 (over $0.05) -> ABORTED (settled).
        fake = _FakeApify([_mk_run("RUNNING", 2), _mk_run("RUNNING", 6),
                           _mk_run("ABORTED", 6)])
        globals()["_req"] = fake.req
        run_actor("acme/act", {"x": 1}, max_charge_usd=0.05)
        check("L3: over-ceiling run is aborted by us", fake.aborted, True)

        # L3b: charge stays under the ceiling -> no abort.
        fake = _FakeApify([_mk_run("RUNNING", 2), _mk_run("SUCCEEDED", 3)])
        globals()["_req"] = fake.req
        run_actor("acme/act", {"x": 1}, max_charge_usd=0.05)
        check("L3: under-ceiling run is NOT aborted", fake.aborted, False)
    finally:
        globals()["_req"] = _real_req
        time.sleep = _real_sleep

    # ==========================================================================
    # TEST 7 — run recovery: reattach() + _run_info_retry(). Fully offline.
    # ==========================================================================
    print("\n=== TEST 7: run recovery (reattach + _run_info_retry) ===")

    class _FakeGet:
        """GET-only fake: successive run_info() calls return successive states."""
        def __init__(self, states):
            self.states = states
            self.i = -1
        ...
        def req(self, method, path, body=None, **kw):
            self.i += 1
            return {"data": self.states[min(self.i, len(self.states) - 1)]}

    class _FlakyGet:
        """Raises `fail_n` transient errors, then returns `then` forever."""
        ...
        def req(self, method, path, body=None, **kw):
            ...
            if self.calls <= self.fail_n:
                raise ApifyCliError("transient")
            return {"data": self.then}

    _real_req2 = _req
    _real_sleep2 = time.sleep
    time.sleep = lambda *a, **k: None
    try:
        # 7a: RUNNING run that finishes -> settled run returned (no re-launch).
        fg = _FakeGet([_mk_run("RUNNING", 2), _mk_run("SUCCEEDED", 3)])
        globals()["_req"] = fg.req
        r = reattach("r1")
        check("reattach RUNNING->SUCCEEDED returns the run",
              r is not None and r.get("status") == "SUCCEEDED", True)

        # 7b: already-SUCCEEDED run -> returned (its result is still fetchable).
        fg = _FakeGet([_mk_run("SUCCEEDED", 5)])
        globals()["_req"] = fg.req
        check("reattach SUCCEEDED returns the run", reattach("r1") is not None, True)

        # 7c/7d: FAILED / ABORTED -> None (nothing paid to recover; re-launch ok).
        fg = _FakeGet([_mk_run("FAILED", 0)])
        globals()["_req"] = fg.req
        check("reattach FAILED -> None", reattach("r1"), None)
        fg = _FakeGet([_mk_run("ABORTED", 1)])
        globals()["_req"] = fg.req
        check("reattach ABORTED -> None", reattach("r1"), None)

        # 7e: unreachable id (every read errors) -> None, not a crash.
        def _boom(method, path, body=None, **kw):
            raise ApifyCliError("transient")
        globals()["_req"] = _boom
        check("reattach unreachable id -> None", reattach("r1"), None)

        # 7f: _run_info_retry rides out 2 transient errors, then succeeds.
        fl = _FlakyGet(2, _mk_run("SUCCEEDED", 4))
        globals()["_req"] = fl.req
        check("_run_info_retry survives 2 transient errors then succeeds",
              _run_info_retry("r1", tries=5, delay=0).get("status"), "SUCCEEDED")

        # 7g: _run_info_retry re-raises when every try fails.
        globals()["_req"] = _boom
        _raised = False
        try:
            _run_info_retry("r1", tries=3, delay=0)
        except ApifyCliError:
            _raised = True
        check("_run_info_retry re-raises after exhausting tries", _raised, True)
    finally:
        globals()["_req"] = _real_req2
        time.sleep = _real_sleep2

    # ==========================================================================
    # TEST 8 — token resolution + auto-persist. Fully offline: no network, no real
    # token. Proves env > ~/.apify/auth.json, that resolving from env WRITES
    # ~/.apify/auth.json so a later run in the same environment does not re-ask (the
    # once-per-environment behavior), and that the write is non-destructive and
    # recovers from a corrupt file.
    # ==========================================================================
    print("\n=== TEST 8: _token() precedence + auto-persist to ~/.apify/auth.json ===")

    import tempfile

    _saved_env = {
        k: os.environ.get(k)
        for k in ("APIFY_TOKEN", "APIFY_API_TOKEN", "HOME")
    }
    ...
    try:
        with tempfile.TemporaryDirectory() as _home:
            _auth = os.path.join(_home, ".apify", "auth.json")
            # Point home at a temp dir (controls ~/.apify/auth.json via expanduser).
            _setenv("HOME", _home)
            ...
            # 8a: env APIFY_TOKEN wins over the file.
            _setenv("APIFY_TOKEN", "env_wins")
            _setenv("APIFY_API_TOKEN", None)
            os.makedirs(os.path.dirname(_auth), exist_ok=True)
            with open(_auth, "w") as f:
                json.dump({"token": "home_tok", "keep": "me"}, f)
            check("env APIFY_TOKEN beats the file", _token(), "env_wins")

            # 8b: resolving from env AUTO-PERSISTED the env token to auth.json, and
            # did NOT clobber the unrelated key -> non-destructive.
            _persisted = _read_auth() or {}
            check("env resolution persisted the token to auth.json",
                  _persisted.get("token"), "env_wins")
            check("auto-persist kept unrelated keys (non-destructive)",
                  _persisted.get("keep"), "me")

            # 8c: with env cleared, the persisted auth.json now answers -> proves a
            # later run in the same environment does not re-ask.
            _setenv("APIFY_TOKEN", None)
            check("auth.json answers after env is gone (no re-ask)", _token(), "env_wins")

            # 8d: nothing anywhere -> None (never a crash).
            _rm(_auth)
            check("no token anywhere -> None", _token(), None)

            # 8e: _persist_token_home recovers from a corrupt auth.json (starts clean
            # rather than raising) and writes the token.
            with open(_auth, "w") as f:
                f.write("{ not json")
            _persist_token_home("recovered")
            check("persist recovers from a corrupt auth.json",
                  (_read_auth() or {}).get("token"), "recovered")
    finally:
        for _k, _v in _saved_env.items():
            _setenv(_k, _v)

    # ==========================================================================
    # TEST 9 — _charge_settled: a zeros-value counts dict is NOT settled (the
    # live App Store bug: {"item": 0} at terminal must not end the settle loop
    # and price the run at $0.00 before the ledger populates). Fully offline.
    # ==========================================================================
    print("\n=== TEST 9: _charge_settled positive-sum requirement (App Store lag bug) ===")
    _ppe = {
        "pricingModel": "PAY_PER_EVENT",
        "pricingPerEvent": {"actorChargeEvents": {
            "apify-default-dataset-item": {"eventPriceUsd": 0.0001},
            "start": {"eventPriceUsd": 0.02},
        }},
    }
    check("PPE zeros-dict counts -> NOT settled (the bug)",
          _charge_settled({"pricingInfo": _ppe,
                           "chargedEventCounts": {"apify-default-dataset-item": 0}}), False)
    check("PPE populated counts -> settled",
          _charge_settled({"pricingInfo": _ppe,
                           "chargedEventCounts": {"apify-default-dataset-item": 100}}), True)
    check("PPE start-fee-only (start:1, zero results) -> settled",
          _charge_settled({"pricingInfo": _ppe, "chargedEventCounts": {"start": 1}}), True)
    check("PPE empty counts -> NOT settled",
          _charge_settled({"pricingInfo": _ppe, "chargedEventCounts": {}}), False)
    check("PPE counts present but NO prices -> NOT settled",
          _charge_settled({"pricingInfo": {"pricingModel": "PAY_PER_EVENT",
                                            "pricingPerEvent": {"actorChargeEvents": {}}},
                           "chargedEventCounts": {"apify-default-dataset-item": 5}}), False)
    check("PAY_PER_COMPUTE usageTotalUsd>0 -> settled",
          _charge_settled({"pricingInfo": {"pricingModel": "PAY_PER_COMPUTE"},
                           "usageTotalUsd": 0.25}), True)
    check("PAY_PER_COMPUTE usageTotalUsd==0 -> NOT settled",
          _charge_settled({"pricingInfo": {"pricingModel": "PAY_PER_COMPUTE"},
                           "usageTotalUsd": 0.0}), False)
    check("no pricingInfo -> NOT settled", _charge_settled({}), False)

    # ==========================================================================
    # TEST 10 — _warn_incomplete: a run that ended ABORTED/FAILED must announce
    # INCOMPLETE + the resume-first recovery, and a SUCCEEDED run must stay
    # silent. This is the fix for the field bug where a bare `[ABORTED]` token
    # was misread as "finished" and a report was written on a truncated corpus.
    # Distinguishes a spend-cap stop from an external abort. Fully offline;
    # reuses _mk_run / _PPE_RESULT from TEST 6 (charge = n_results × $0.01).
    # ==========================================================================
    print("\n=== TEST 10: _warn_incomplete announces partial runs + resume ===")

    def _warn_out(run, actor="acme/act", ceiling=None, we_aborted=False):
        buf = io.StringIO()
        _old = sys.stderr
        sys.stderr = buf
        try:
            _warn_incomplete(run, actor, ceiling, we_aborted)
        finally:
            sys.stderr = _old
        return buf.getvalue()

    # 10a: a clean run says nothing.
    check("SUCCEEDED -> no warning",
          _warn_out(_mk_run("SUCCEEDED", 100)) == "", True)

    # 10b: an external abort (spend far under the cap) warns, points at RESUME,
    # and is NOT framed as a spend-cap stop.
    _ext = _warn_out(_mk_run("ABORTED", 2), ceiling=1.00)  # $0.02 of $1.00
    check("external ABORTED -> INCOMPLETE warning", "RUN INCOMPLETE" in _ext, True)
    check("external ABORTED -> tells caller to RESUME", "RESUME" in _ext, True)
    check("external ABORTED -> not framed as a cap stop", "spend cap" not in _ext, True)

    # 10c: a spend-cap stop (charge ~ the ceiling) warns AND says raise the cap.
    _cap = _warn_out(_mk_run("ABORTED", 10), ceiling=0.10)  # $0.10 of $0.10
    check("cap-stop ABORTED -> says raise the budget cap",
          "raise the budget cap" in _cap, True)

    # 10d: our own L3 abort (we_aborted=True) is a cap stop even before the
    # ledger catches up to the ceiling.
    _l3 = _warn_out(_mk_run("ABORTED", 1), ceiling=5.00, we_aborted=True)
    check("we_aborted -> treated as a cap stop", "raise the budget cap" in _l3, True)

    # 10e: FAILED also warns (any non-SUCCEEDED terminal is partial).
    check("FAILED -> INCOMPLETE warning",
          "RUN INCOMPLETE" in _warn_out(_mk_run("FAILED", 0)), True)

    # ==========================================================================
    # TEST 11 — actor-minimum floor (A) + L2/L3 decoupling (B). Some actors set
    # minimalMaxTotalChargeUsd; passing a cap below it makes Apify REJECT the run
    # at start. We floor the Apify cap at that minimum WITHOUT loosening our own
    # tight poll-and-abort. Fully offline; a path-routing fake serves actor-info,
    # start, poll and abort, and lets us confirm the min lookup is cached.
    # ==========================================================================
    print("\n=== TEST 11: actor-min floor + L2/L3 decoupling ===")

    class _FloorFake:
        def __init__(self, states, floor=0.5):
            self.states = states
            self.gi = 0
            self.start_path = None
            self.aborted = False
        ...
        def req(self, method, path, body=None, **kw):
            if method == "GET" and path.startswith("/acts/"):
                self.acts_gets += 1
                return {"data": {"pricingInfos": [
                    {"pricingModel": "PAY_PER_EVENT",
                     "minimalMaxTotalChargeUsd": self.floor}]}}
            if method == "POST" and "/abort" in path:
                self.aborted = True
                return {"data": self.states[min(self.gi, len(self.states) - 1)]}
            if method == "POST":
                ...
                self.gi = 0
                return {"data": self.states[0]}
            self.gi += 1  # GET /actor-runs/... -> advance run state
            return {"data": self.states[min(self.gi, len(self.states) - 1)]}

    _real_req3 = _req
    _real_sleep3 = time.sleep
    time.sleep = lambda *a, **k: None
    try:
        # 11a (A): forecast ceiling $0.135 < floor $0.50 -> Apify cap floored to 0.50.
        _ACTOR_MIN_CACHE.clear()
        fake = _FloorFake([_mk_run("SUCCEEDED", 3, rid="rf")], floor=0.5)
        globals()["_req"] = fake.req
        run_actor("floor/act", {"x": 1}, max_charge_usd=0.135)
        check("A: Apify cap floored to the actor minimum (0.5000)",
              "maxTotalChargeUsd=0.5000" in (fake.start_path or ""), True)

        # 11b (A): the min lookup is cached -> a second run does not re-GET /acts.
        before = fake.acts_gets
        run_actor("floor/act", {"x": 1}, max_charge_usd=0.135)
        check("A: actor-min lookup is cached (no second /acts GET)",
              fake.acts_gets, before)

        # 11c (B): L3 aborts at the TIGHT $0.05 ceiling, not the $0.50 Apify cap.
        _ACTOR_MIN_CACHE.clear()
        fake = _FloorFake([_mk_run("RUNNING", 2, rid="rf"),
                           _mk_run("RUNNING", 6, rid="rf"),   # $0.06 > $0.05 -> abort
                           _mk_run("ABORTED", 6, rid="rf")], floor=0.5)
        globals()["_req"] = fake.req
        run_actor("floor/act", {"x": 1}, max_charge_usd=0.05)
        check("B: L3 aborts at the tight $0.05 despite the $0.50 Apify cap",
              fake.aborted, True)

        # 11d (B): under the tight ceiling -> NOT aborted (even far under $0.50).
        _ACTOR_MIN_CACHE.clear()
        fake = _FloorFake([_mk_run("RUNNING", 1, rid="rf"),
                           _mk_run("SUCCEEDED", 3, rid="rf")], floor=0.5)  # $0.03 < $0.05
        globals()["_req"] = fake.req
        run_actor("floor/act", {"x": 1}, max_charge_usd=0.05)
        check("B: under the tight ceiling -> NOT aborted", fake.aborted, False)

        # 11e: an actor with NO floor -> cap stays at the forecast ceiling.
        _ACTOR_MIN_CACHE.clear()
        fake = _FloorFake([_mk_run("SUCCEEDED", 3, rid="rf")], floor=0)
        globals()["_req"] = fake.req
        run_actor("nofloor/act", {"x": 1}, max_charge_usd=0.135)
        check("no-floor actor: cap stays at the forecast ceiling (0.1350)",
              "maxTotalChargeUsd=0.1350" in (fake.start_path or ""), True)
    finally:
        globals()["_req"] = _real_req3
