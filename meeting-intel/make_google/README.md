...
# Meeting Intel - Make.com + Google Calendar Variant

This folder contains the Google Calendar variant of Meeting Intel: a Make.com scenario that reads your Google Calendar, researches the external participants of today's meetings using Apify and OpenAI, and emails you a formatted HTML intelligence brief via Gmail before each meeting.
...
1. Reads your Google Calendar for today's events via Google's Calendar API
...
6. Emails the brief to the address you specify via Gmail
...
- **Google account** - used for both reading your Google Calendar and sending the brief via Gmail; the same Google account serves both purposes, connected twice in Make

---

## Quick start

> **Critical setup step - read before you begin:** The Google Calendar module in Make.com requests no calendar permissions by default. If you miss the scope step during connection setup, every run will fail with a 403 error. This is the single most common setup mistake with this variant.

1. In Make.com, go to Scenarios and click **Create a new scenario**
2. Click the three-dot menu and select **Import Blueprint**
3. Paste the contents of `meeting_intel_google.json` and click **Save**
4. Set up your four connections:
   - **Google Calendar connection (module 1):** Click Add, then click **Show advanced settings**, add the scope `https://www.googleapis.com/auth/calendar`, and then sign in. This step is mandatory - see the User Manual for the full procedure.
   - **Gmail connection (module 24):** Click Add and sign in normally. This is a separate connection type from the Calendar one, even if it is the same Google account.
   - **Apify** and **OpenAI:** enter your API tokens.
5. Configure the five settings described in the user manual (modules 2, 24, 23, and 18)
6. Click **Run once**
...
| File | Purpose |
|---|---|
| `meeting_intel_google.json` | The Make.com scenario blueprint to import |
